function ASPSnippetsPager(a, b)
{
    ...
    if (h > 1) {
        ...
        if (k > b.RecordCount) { k = b.RecordCount; } i = "<b>Records " + (j === 0 ? 1 : j) + " - " + k + " of " + b.RecordCount + "</b> ";
        if (b.PageIndex > 1) { i += c.replace("{0}", "<<").replace("{1}", "1"); i += c.replace("{0}", "<").replace("{1}", b.PageIndex - 1); }
        for (var l = e; l <= f; l++) {
            if (l === b.PageIndex) { i += d.replace("{0}", l); }
            else { i += c.replace("{0}", l).replace("{1}", l); }
        }
        if (b.PageIndex < h) { i += c.replace("{0}", ">").replace("{1}", b.PageIndex + 1); i += c.replace("{0}", ">>").replace("{1}", h); }
    } a.html(i);
    try {
        a[0].disabled = false;
    }
