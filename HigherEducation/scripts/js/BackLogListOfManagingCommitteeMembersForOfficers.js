...
function BindAllBackLogDetailsForOfficers() {
    ...
    if (SocietyTransID === "") {
        ...
        debugger;
        if ($('#SubCodeVal').val() == 31 || $('#SubCodeVal').val() == 41 || $('#SubCodeVal').val() == 0) {
            $('.PacsContent').hide();
            $('.WithoutPacs').show();
        }
        else
...
function loadBackLogDetails() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $("#totalnomember").val(getbySociety.Noofmember);         
            $("#CommunityofSociety").val(getbySociety.CommunityOfSocietyId);          
            $("#SubClassOfSociety").val(getbySociety.KindOfSocietyId);                
            $("#SubCodeVal").val(getbySociety.SubClassSocietyCode);
            $("#IsCertificateObtained").val(getbySociety.IsCertificateObtained);

            if (getbySociety.IsCertificateObtained === "Yes") {
                $("#divdateofoccupcertificate").css("display", "block");
            }           
            $("#dateofoccupcertificate").val(getbySociety.DateofOccupation);
            $("#IsAmountLandCost").val(getbySociety.IsAmountLandCost);
            $("#IsSocietyStatus").val(getbySociety.IsSocietyStatus);
            $("#IsConveyance").val(getbySociety.IsConveyance);
            $("#IsResumed").val(getbySociety.IsResumed);
            $("#dateofallotment").val(getbySociety.DateofAllotment);
            $("#IsBuildingApproved").val(getbySociety.IsBuildingApproved);
            if (getbySociety.IsBuildingApproved === "Yes") {
                $("#divBuildingAppDate").css("display", "block");
            }
            $("#BuildingAppDate").val(getbySociety.DateofBuildingApproval);
            $("#IsSocietyLitigation").val(getbySociety.IsSocietyLitigation);
            $("#IsBoaBod").val(getbySociety.IsBoaBod);
            if (getbySociety.IsBoaBod === "BOA") {
                $("#divdateofboabod").css("display", "block");
            }
            $("#dateofboabod").val(getbySociety.DateofBoaAppointment);   
            $("#RegisteredAddress").val(getbySociety.RegisteredAddress);
            $("#HouseNoSectorNoRoad").val(getbySociety.HouseNoSectorNoRoad);
            $("#PostOffice").val(getbySociety.PostOffice);
            $("#PostalCode").val(getbySociety.PostalCode);
            $("#membernotavailable").val(getbySociety.membernotavailable);
            if ($('#SubCodeVal').val() == 31 || $('#SubCodeVal').val() == 41 || $('#SubCodeVal').val() == 0) {
                $('.PacsContent').hide();
                $('.WithoutPacs').show();
            }
            ...
            if ($("#IsCertificateObtained").val() == null) {
                $("#IsCertificateObtained").val("No");
                $("#IsAmountLandCost").val("No");
                $("#IsSocietyStatus").val("Constructed");
                $("#IsConveyance").val("No");
                $("#IsResumed").val("No");
                $("#IsBuildingApproved").val("No");
                $("#IsSocietyLitigation").val("No");
                $("#IsBoaBod").val("None");
                $("#dateofoccupcertificate").val("");
                $("#dateofallotment").val("");
                $("#BuildingAppDate").val("");
                $("#dateofboabod").val("");

            }
            if ($("#dateofallotment").val() == 'NaN-NaN-NaN') {
                $("#dateofallotment").val("");
            }
            if ($("#BuildingAppDate").val() == 'NaN-NaN-NaN') {
                $("#BuildingAppDate").val("");
            }
            if ($("#dateofboabod").val() == 'NaN-NaN-NaN') {
                $("#dateofboabod").val("");
            }
            if ($("#dateofoccupcertificate").val() == 'NaN-NaN-NaN') {
                $("#dateofoccupcertificate").val("");
            }
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(result.getRecord, function (key, item) {
                ...
                html += '<td hidden="hidden">' + item.MemberSNo + '</td>';
                html += '<td>' + i + '</td>';
                //html += '<td><img src="' + item.base64 + '" width="52px;" height="75px;"/></td>';
                html += '<td>' + item.MemberName + '</td>';
...
function Bck_loadgetbySocietyMemberID(MemberSNo) {
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#dateApproval1').val(getbySocietyMemberID.dateApproval);
            $('#MembershipNo').val(getbySocietyMemberID.MembershipNo);
            $('#PFTCode').val(getbySocietyMemberID.PFTCode);    
            $('#PFTValue').val(getbySocietyMemberID.PFTValue);
            $('#TowerNo').val(getbySocietyMemberID.TowerNo);
            $('#IsMortgage').val(getbySocietyMemberID.IsMortgage);
            $('#IsLitigation').val(getbySocietyMemberID.IsLitigation);
            $('#CourtName').val(getbySocietyMemberID.CourtName);
            $('#CaseInBrief').val(getbySocietyMemberID.CaseInBrief);
            $('#CourtNameId').val(getbySocietyMemberID.CourtNameId);
            $('#MortgagedDetail').val(getbySocietyMemberID.MortgagedDetail);
            $('#MemberSNo').val(getbySocietyMemberID.MemberSNo);
            $('#MemberName').val(getbySocietyMemberID.MemberName);
            $('#FatherName').val(getbySocietyMemberID.FatherName);
            $('#BckManagingMemberRelationship').val(getbySocietyMemberID.ManagingMemberRelationship);       
            $('#GenderOfSocietyMember').val(getbySocietyMemberID.Gender);
            $('#img').attr('src', getbySocietyMemberID.base64);          
            var Age = getbySocietyMemberID.Age;
            if (Age === 0) {
                $('#Age').val("");
            }
            ...
            if ($("#PFTCode").val() == "Plot No") {
                $("#HidePFT").show();
            }
            else {
                $("#HidePFT").hide();
            }    
            if ($("#PFTCode").val() == "Flat No") {
                $("#HidePFT").show();
            }
            else {
                $("#HidePFT").hide();
            }     
            if ($("#IsMortgage").val() == "Yes") {
                $("#HideMortgaged").show();
...
function parseJsonDate1(jsonDate) {
    ...
    return date.getDate() + "-" + finalmonth + '-' + date.getFullYear();
}
