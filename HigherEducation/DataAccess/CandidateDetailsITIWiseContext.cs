...
{
    public class CandidateDetailsITIWiseContext
    {
        ...
        public DataSet getStudentDetailsList(String CollegeType,String CollegeId, String CourseId, String SectionId)
        {
            DataSet dt = new DataSet();
            try
            {
                ...
                MySqlDataAdapter da = new MySqlDataAdapter(cmd);
                cmd.Parameters.AddWithValue("p_CollegeType", CollegeType);
        ...
        public DataSet getStudentDetailsList_2022(String CollegeId, String CourseId, String SectionId)
        {
            DataSet dt = new DataSet();
            try
            {
                MySqlCommand cmd = new MySqlCommand("GetCandidateDetalsITIWise_2022", connection_ReadOnly)
                {
                    CommandTimeout = 120,
                    CommandType = CommandType.StoredProcedure,
                };
            ...
            {
                logger = LogManager.GetLogger("databaseLogger");
                logger.Error(ex, "Error occured in HigherEducation.DataAccess.ReportsContext.getStudentDetailsList_2022()");
            }
            finally
            {
                if (connection_ReadOnly.State == ConnectionState.Open)
