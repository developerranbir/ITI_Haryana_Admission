...
namespace HigherEducation.DataAccess
{
    public class SummaryContext
    {
        ...
        public DataSet getTotalSeatList(String CollegeType,String CollegeId, String CourseId, String SectionId)
        {
            DataSet dt = new DataSet();
            try
            {
                MySqlCommand cmd = new MySqlCommand("GetSummaryTradewise", connection_ReadOnly)
                {
                    CommandTimeout = 120,
            ...
            catch (Exception ex)
            {
                ...
                logger.Error(ex, "Error occured in HigherEducation.DataAccess.SummaryContext.getTotalSeatList()");
            }
            finally
            {
        ...
        public DataTable GetTradeListByITI(String collegeId)
        {
            ...
            {
                logger = LogManager.GetLogger("databaseLogger");
                logger.Error(ex, "Error occured in HigherEducation.DataAccess.SummaryContext.GetTradeListByITI()");
            }
            finally
            {
                if (connection_ReadOnly.State == ConnectionState.Open)
        ...
        public DataTable getCollegeListAsPerCollegeType(String collegeType)
        {
            ...
            {
                logger = LogManager.GetLogger("databaseLogger");
                logger.Error(ex, "Error occured in HigherEducation.DataAccess.SummaryContext.getCollegeListAsPerCollegeType()");
            }
            finally
            {
                if (connection_ReadOnly.State == ConnectionState.Open)
        ...
        public DataSet GetSectionList(String CollegeId, String CourseId)
        {
            ...
            {
                logger = LogManager.GetLogger("databaseLogger");
                logger.Error(ex, "Error occured in HigherEducation.DataAccess.SummaryContext.GetSectionList()");
            }
            finally
            {
                if (connection_ReadOnly.State == ConnectionState.Open)
        ...
        public DataSet getCourseWiseList(String CollegeType, String CollegeId, String CourseId)
        {
            DataSet dt = new DataSet();
            try
            {
                MySqlCommand cmd = new MySqlCommand("GetSummaryCourseWise", connection_ReadOnly)
                {
                    CommandTimeout = 120,
                    CommandType = CommandType.StoredProcedure,
                };
            ...
            {
                logger = LogManager.GetLogger("databaseLogger");
                logger.Error(ex, "Error occured in HigherEducation.DataAccess.SummaryContext.getCourseWiseList()");
            }
            finally
            {
                if (connection_ReadOnly.State == ConnectionState.Open)
